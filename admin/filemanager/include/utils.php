...
function isUploadDir($path, $config) {
    ...
    if (!isset($path) || !isset($config['current_path']) || !isset($config['thumbs_base_path'])) {
        return false; // Trả về false nếu không hợp lệ
    }

    // Lấy đường dẫn thực tế và kiểm tra sự tồn tại của các thư mục
    $realPath = realpath($path);
    $uploadDir = realpath($config['current_path']);
    $thumbsDir = realpath($config['thumbs_base_path']);

    // So sánh đường dẫn với thư mục tải lên và thư mục hình thu nhỏ
    if ($realPath && ($realPath === $uploadDir || $realPath === $thumbsDir)) {
        return true;
    }
