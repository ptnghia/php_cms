        ...
        if (!isset($_GET['id_loai'])) {
            ...
            if ($id_code = $d->insert($data0)) {
                foreach (get_json('lang') as $key => $value) {
                    ...
                    $data_cate['noi_dung']  =   $d->clear($_POST['noi_dung'][$key]);
	        ...
	        <div class="row">
	            <div class="col-sm-8">
	                <div class="box box-primary">
	                    ...
	                    <?php $danhmuc_home = $d->o_fet("select * from #_category_noidung where lang='" . LANG . "'  and type = 0 order by id ASC"); ?>
	                                ...
	                                <tr>
	                                    <td class="text-center">
	                                        ...
	                                        <button type="button" class="btn btn-xs btn-success"
	                                            onclick="$('.noi_dun_<?= $value['id'] ?>').slideToggle();">
	                                            <i class="fa fa-long-arrow-down"></i> Chi tiết (<?= count($hinhanh) ?>)
	                                        </button>
	                                        <?php } ?>
	                                        <a href="index.php?p=noi-dung&a=add&parent=<?= $value['id_code'] ?>"
	                                            class="btn btn-xs btn-primary">Thêm nội dung</a>
	                                    </td>
	                                    <td><?= $value['ten'] ?></td>
	                                    <td class="text-center">
	                                    ...
	                                    <td class="text-center">
	                                        <input <?php if ($d->checkPermission_edit($id_module) == 0) { ?> disabled
	                                            <?php } ?> class="chk_box" type="checkbox"
	                                            onclick="on_check(this,'#_category_noidung','hien_thi','<?= $value['id_code'] ?>')"
	                                            <?php if ($value['hien_thi'] == 1) echo 'checked="checked"'; ?>>
	                                    </td>
	                                    <td class="text-center">
	                                        <a href="index.php?p=<?= $_GET['p'] ?>&a=man&id_loai=<?= $value['id_code'] ?>"
	                                        ...
	                                        <a href="index.php?p=<?= $_GET['p'] ?>&a=man&delete=<?= $value['id_code'] ?>"
	                                            onClick="if(!confirm('Xác nhận xóa?')) return false;"
	                                            class="label label-danger" title="Xóa"><i
	                                                class="glyphicon glyphicon-remove"></i> Xóa</a>
	                                        <?php } ?>
	                                    </td>
	                                </tr>
	                                <?php foreach ($hinhanh as $key2 => $value2) { ?>
	                                <tr <?= $_GET['parent'] == $value['id_code'] ? '' : 'style="display: none"' ?>
	                                    class="noi_dun_<?= $value['id'] ?>">
	                                    <td class="text-center">
	                                        <?php if ($d->checkPermission_edit($id_module) == 1) { ?>
	                                        <input type="number" value="<?= $value2['so_thu_tu'] ?>" class="a_stt"
	                                            data-table="#_content" data-col="so_thu_tu"
	                                            data-id="<?= $value2['id_code'] ?>" />
	                                        <?php } else { ?>
	                                    ...
	                                    <td class="text-center">
	                                        ...
	                                        <img src="../img_data/images/<?= $value2['hinh_anh'] ?>"
	                                            style="height:50px;width: 170px;object-fit: contain;">
	                                        <?php } ?>
	                                    </td>
	                                    <td class="text-center">
	                                        <input class="chk_box"
	                                            <?php if ($d->checkPermission_edit($id_module) == 0) { ?> disabled
	                                            <?php } ?> type="checkbox"
	                                            onclick="on_check(this,'#_content','hien_thi','<?= $value2['id_code'] ?>')"
	                                            <?php if ($value2['hien_thi'] == 1) echo 'checked="checked"'; ?>>
	                                    </td>
	                                    <td class="text-center">
	                                        <a href="index.php?p=<?= $_GET['p'] ?>&a=edit&id=<?= $value2['id_code'] ?>&parent=<?= $value['id_code'] ?>"
	                                            class="label label-warning" title="Sửa"><i
	                                                class="glyphicon glyphicon-edit"></i> Sửa</a>
	                                        <?php if ($d->checkPermission_dele($id_module) == 1) { ?>
	                                        <a href="index.php?p=<?= $_GET['p'] ?>&a=delete&id=<?= $value2['id_code'] ?>&parent=<?= $value['id_code'] ?>"
	                                            onClick="if(!confirm('Xác nhận xóa?')) return false;"
	                                            class="label label-danger" title="Xóa"><i
	                                                class="glyphicon glyphicon-remove"></i> Xóa</a>
	                                        <?php } ?>
	                        ...
	                        <form method="POST" action="" enctype="multipart/form-data">
	                            <div class="form-group m0 hinh_anh">
	                                ...
	                                <span class="box-img2">
                                        ...
                                        ?>
	                                    <img onerror="this.src='img/no-image.png';"
	                                        src="../img_data/images/<?php echo $row0['hinh_anh'] ?>" id="review_hinh_anh"
	                                        alt="NO PHOTO" />
	                                    <button class="btn btn-xs btn-danger" type="button"
	                                        onclick="xoa_img('_category_noidung','hinh_anh', '<?= $_GET['id'] ?>','')"><i
	                                            class="fa fa-trash"></i></button>
	                                    <?php } else { ?>
	                                    <img src="img/no-image.png"
	                                        style="max-width: 100%;max-height: 100%;object-fit: contain;"
	                                        id="review_hinh_anh" alt="NO PHOTO" />
	                                    <?php } ?>
	                                    <input type="hidden" value="<?= $row['hinh_anh'] ?>" name="hinh_anh" id="hinh_anh"
	                                        class=" form-control">
	                                    <a href="filemanager/dialog.php?type=1&field_id=hinh_anh&relative_url=1&multiple=0"
	                                        class="btn btn-upload2 iframe-btn"> <i class="fa fa-upload"
	                                            aria-hidden="true"></i>Chọn hình ảnh</a>
	                                </span>
	                            </div>
	                            ...
	                            <ul id="myTabs" class="nav nav-tabs" role="tablist">
	                                ...
	                                <li role="presentation" <?php if ($key == 0) { ?>class="active" <?php } ?>>
	                                    <a href="#seo_<?= $value['code'] ?>" id="home-tab" role="tab" data-toggle="tab"
	                                        aria-controls="seo_<?= $value['code'] ?>"
	                                        aria-expanded="true"><?= $value['name'] ?></a>
	                                </li>
	                                <?php } ?>
	                            </ul>
	                            <?php } ?>
	                            <div id="myTabContent" class="tab-content">
                                    ...
                                    ?>
	                                <div role="tabpanel"
	                                    class="tab-pane fade in <?php if ($key == 0) { ?> active <?php } ?>"
	                                    id="seo_<?= $value['code'] ?>" aria-labelledby="seo_<?= $value['code'] ?>">
	                                    <div class="form-group">
	                                        <label>Tên</label>
	                                        <div class="row m-5">
	                                            <div class="col-sm-8 p5">
	                                                <input type="text" value="<?= $row['ten'] ?>"
	                                                    placeholder="Nhập danh mục mới" name="ten[]"
	                                                    class="form-control" />
	                                            </div>
	                                            <div class="col-sm-4 p5">
	                                                <select class=" form-control" name="heading">
	                                                    <option <?= $row['heading'] == 'div' ? 'selected' : '' ?>
	                                                        value="div">Bình thường (DIV)</option>
	                                                    <option <?= $row['heading'] == 'h1' ? 'selected' : '' ?>
	                                                        value="h1">Heading 1</option>
	                                                    <option <?= $row['heading'] == 'h2' ? 'selected' : '' ?>
	                                                        value="h2">Heading 2</option>
	                                                    <option <?= $row['heading'] == 'h3' ? 'selected' : '' ?>
	                                                        value="h3">Heading 3</option>
	                                                    <option <?= $row['heading'] == 'h4' ? 'selected' : '' ?>
	                                                        value="h4">Heading 4</option>
	                                                    <option <?= $row['heading'] == 'h5' ? 'selected' : '' ?>
	                                                        value="h5">Heading 5</option>
	                                                </select>
	                                            </div>
	                                        </div>
	                                    ...
	                                    <div class="form-group">
	                                        ...
	                                        <input type="text" value="<?= $row['link'] ?>" placeholder="Liên kết"
	                                            name="link[]" class="form-control" />
	                                        <div class="checkbox">
	                                            <label class="checkbox-inline">
	                                                <input name="nofollow"
	                                                    <?php if ($row['nofollow'] == 1) echo 'checked="checked"'; ?>
	                                                    type="checkbox"> Nofollow
	                                            </label>
	                                            <label class="checkbox-inline">
	                                                <input name="target"
	                                                    <?php if ($row['target'] == 1) echo 'checked="checked"'; ?>
	                                                    type="checkbox"> Mở cửa số mới (_blank)
	                                            </label>
	                                        </div>
	                                    </div>
	                                    <!--div class="form-group">
	                                    ...
	                                    <div class="form-group">
	                                        ...
	                                        <textarea class="form-control" placeholder="Nhập nội dung" rows="5"
	                                            id="noi_dung_<?= $value['code'] ?>"
	                                            name="noi_dung[]"><?= $row['noi_dung'] ?></textarea>
	                                        <script>
